...
export class DataTypeDetectionTest {
  static testDetectionLogic() {
    ...
    testCases.forEach(testCase => {
      ...
      console.log(
        `Expected: ${testCase.expected}, Detected: ${detected} ${passed ? '✅' : '❌'}`
      );
    ...
    console.log(
      `📊 Detection Tests: ${passedTests}/${testCases.length} passed`
    );
    console.log(
  ...
  static testRevenueNormalization() {
    ...
    console.log(
      `Revenue normalization: ${isValid ? '✅ WORKING' : '❌ FAILED'}`
    );
    return isValid;
  }
  ...
  static runAllTests() {
    ...
    console.log('='.repeat(60));
    console.log(
      `Data Type Detection: ${detectionTest ? '✅ WORKING' : '❌ BROKEN'}`
    );
    console.log(
      `Revenue Normalization: ${normalizationTest ? '✅ WORKING' : '❌ BROKEN'}`
    );
