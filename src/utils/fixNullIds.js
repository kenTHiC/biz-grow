...
export class FixNullIds {
  static async fixAllData() {
    ...
    try {
      ...
      console.log(
        'Revenues:',
        dataStore.revenues.map(r => ({ id: r.id, amount: r.amount }))
      );
      console.log(
        'Expenses:',
        dataStore.expenses.map(e => ({ id: e.id, amount: e.amount }))
      );
      console.log(
        'Customers:',
        dataStore.customers.map(c => ({ id: c.id, name: c.name }))
      );
      ...
      const hasNullRevenues = revenueIds.some(
        id => id === null || id === undefined
      );
      const hasNullExpenses = expenseIds.some(
        id => id === null || id === undefined
      );
      const hasNullCustomers = customerIds.some(
        id => id === null || id === undefined
      );

      console.log('\nID Validation:');
      console.log(
        `Revenues have null IDs: ${hasNullRevenues ? '❌ YES' : '✅ NO'}`
      );
      console.log(
        `Expenses have null IDs: ${hasNullExpenses ? '❌ YES' : '✅ NO'}`
      );
      console.log(
        `Customers have null IDs: ${hasNullCustomers ? '❌ YES' : '✅ NO'}`
      );

      const allFixed =
        !hasNullRevenues && !hasNullExpenses && !hasNullCustomers;
  ...
  static async manuallyFixData() {
    ...
    try {
      ...
      const expenses = JSON.parse(
        localStorage.getItem('biz-grow-expenses') || '[]'
      );
      const customers = JSON.parse(
        localStorage.getItem('biz-grow-customers') || '[]'
      );

      console.log('Current localStorage data:');
      console.log(
        'Revenues:',
        revenues.map(r => ({ id: r.id, amount: r.amount }))
      );
      console.log(
        'Expenses:',
        expenses.map(e => ({ id: e.id, amount: e.amount }))
      );
      console.log(
        'Customers:',
      ...
      localStorage.setItem(
        'biz-grow-customers',
        JSON.stringify(fixedCustomers)
      );

      console.log('\n✅ Data fixed and saved to localStorage');
      console.log(
        'Fixed revenues:',
        fixedRevenues.map(r => ({ id: r.id, amount: r.amount }))
      );
      console.log(
        'Fixed expenses:',
        fixedExpenses.map(e => ({ id: e.id, amount: e.amount }))
      );
      console.log(
        'Fixed customers:',
        fixedCustomers.map(c => ({ id: c.id, name: c.name }))
      );
