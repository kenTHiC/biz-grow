...
export class QuickDeleteTest {
  ...
  static async clearTestData() {
    ...
    try {
      ...
      // Clear test revenues
      const revenues = await Revenue.list();
      const testRevenues = revenues.filter(r => r.source && r.source.includes('Test Revenue'));
      ...
      const testExpenses = expenses.filter(e => e.vendor && e.vendor.includes('Test Vendor'));
