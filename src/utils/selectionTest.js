...
export class SelectionTest {
  static testSelectionLogic() {
    ...
    const success = selectedItems.length === 2 && 
  ...
  static testCheckboxState() {
    ...
    items.forEach(item => {
      ...
      console.log(`Item ${item.id} checkbox should be: ${isChecked ? 'CHECKED' : 'UNCHECKED'}`);
    });
  ...
  static testSelectAllLogic() {
    ...
    console.log(`Select all logic test: ${success ? '✅ PASSED' : '❌ FAILED'}`);
  ...
  static runAllTests() {
    ...
    console.log('=' .repeat(60));
    console.log(`Individual Selection Logic: ${selectionTest ? '✅ WORKING' : '❌ BROKEN'}`);
    console.log(`Checkbox State Calculation: ${checkboxTest ? '✅ WORKING' : '❌ BROKEN'}`);
    console.log(`Select All Logic: ${selectAllTest ? '✅ WORKING' : '❌ BROKEN'}`);
    ...
    if (allPassed) {
      ...
      console.log('The issue is likely in React state management or event handling');
