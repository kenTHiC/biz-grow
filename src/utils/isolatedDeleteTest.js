...
export class IsolatedDeleteTest {
  ...
  static async testReportsPageLoadData() {
    ...
    try {
      ...
      if (beforeData.revenueData.length > 0) {
        ...
        const success =
          afterData.revenueData.length === beforeData.revenueData.length - 1;
        console.log(`\nLoadData test: ${success ? '✅ PASSED' : '❌ FAILED'}`);
        console.log(
          `Before: ${beforeData.revenueData.length}, After: ${afterData.revenueData.length}`
        );
  ...
  static async runAllTests() {
    ...
    console.log('='.repeat(60));
    console.log(
      `DataStore Layer: ${dataStoreTest ? '✅ WORKING' : '❌ BROKEN'}`
    );
    console.log(`Entity Layer: ${entityTest ? '✅ WORKING' : '❌ BROKEN'}`);
    console.log(
      `LoadData Function: ${loadDataTest ? '✅ WORKING' : '❌ BROKEN'}`
    );
    console.log(
      `Selection Logic: ${selectionTest ? '✅ WORKING' : '❌ BROKEN'}`
    );

    const allPassed =
      dataStoreTest && entityTest && loadDataTest && selectionTest;
