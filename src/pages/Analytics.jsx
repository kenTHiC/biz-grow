...
import { Analytics as AnalyticsService, Revenue, Expense, Customer } from "@/entities/all";
import { TrendingUp, TrendingDown, DollarSign, Users, Calendar, Target, BarChart3, PieChart } from "lucide-react";
import { format, subDays, startOfMonth, endOfMonth, isWithinInterval } from "date-fns";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { PieChart as RechartsPieChart, Pie, Cell, BarChart, Bar, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
    ...
    return (
      <div className="p-8">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-slate-600">No data available for the selected date range.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
              <h1 className="text-4xl font-bold text-slate-900 mb-2">Business Analytics</h1>
              <p className="text-slate-600 text-lg">Deep insights into your business performance</p>
            </div>
          </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">${analytics.totalExpenses.toLocaleString()}</p>
                  <p className="text-xs text-amber-600 mt-1">+5.2% vs last period</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">${analytics.netProfit.toLocaleString()}</p>
                  <p className={`text-xs mt-1 ${analytics.netProfit > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    {analytics.netProfit > 0 ? '+18.3%' : '-8.1%'} vs last period
                  </p>
                </div>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${analytics.netProfit > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">{analytics.activeCustomers}</p>
                  <p className="text-xs text-emerald-600 mt-1">+23.1% vs last period</p>
                </div>
          ...
          >
            <Card className="bg-white/80 backdrop-blur-sm border-slate-200/60 shadow-lg">
              ...
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={expenseByCategory} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
