...
const APP_VERSION = (() => {
  try {
    ...
    if (version) {
      ...
      console.warn(
        '⚠️ Could not read version from package.json, using fallback version'
      );
      return '1.3.3';
    }
  } catch (error) {
...
const fixNullIds = (items, itemType) => {
  ...
  if (hasNullIds) {
    console.log(
      `Fixed ${itemType} with null IDs. New data:`,
      fixedItems.map(i => ({ id: i.id, amount: i.amount }))
    );
  }
...
const fixDuplicateIds = (items, itemType) => {
  ...
  const fixed = items.map(item => {
    ...
    if (isNaN(idNum) || seen.has(idNum)) {
      ...
      changed = true;
      console.log(
        `Reassigning duplicate/invalid ${itemType} ID ${rawId} -> ${newItem.id}`
      );
      seen.add(newItem.id);
      return newItem;
    }
  ...
  if (changed) {
    console.log(
      `${itemType}: Duplicate IDs fixed. IDs:`,
      fixed.map(i => i.id)
    );
  }
...
class DataStore {
  constructor() {
    ...
    this.customers = fixDuplicateIds(
      fixNullIds(loadFromStorage(STORAGE_KEYS.CUSTOMERS, []), 'customers'),
      'customers'
    );
    this.revenues = fixDuplicateIds(
      fixNullIds(loadFromStorage(STORAGE_KEYS.REVENUES, []), 'revenues'),
      'revenues'
    );
    this.expenses = fixDuplicateIds(
      fixNullIds(loadFromStorage(STORAGE_KEYS.EXPENSES, []), 'expenses'),
      'expenses'
    );
  ...
  getCustomers(sortBy = '-acquisition_date') {
    ...
    const validCustomers = this.customers.filter(
      customer =>
        customer &&
        customer.id !== null &&
    ...
    if (sortBy === '-acquisition_date') {
      sorted.sort(
        (a, b) => new Date(b.acquisition_date) - new Date(a.acquisition_date)
      );
    }
    return sorted;
  }
  ...
  addCustomer(customerData) {
    ...
    const newCustomer = {
      ...
      acquisition_date:
        customerData.acquisition_date || format(new Date(), 'yyyy-MM-dd'),
  ...
  updateCustomer(id, customerData) {
    ...
    if (index !== -1) {
      ...
      if (
        customerData.email &&
        customerData.email.toLowerCase() !== currentCustomer.email.toLowerCase()
      ) {
        const existingCustomer = this.customers.find(
          c =>
      ...
      const updatedCustomer = {
        ...
        total_value: parseFloat(customerData.total_value) || 0,
        acquisition_date:
          customerData.acquisition_date || currentCustomer.acquisition_date,
  ...
  deleteRevenue(id) {
    ...
    console.log(
      'Current revenues:',
      this.revenues.map(r => ({ id: r.id, amount: r.amount }))
    );
  ...
  deleteExpense(id) {
    ...
    console.log(
      'Current expenses:',
      this.expenses.map(e => ({ id: e.id, amount: e.amount }))
    );
  ...
  async importData(data, options = {}) {
    const { merge = false, validateData = true, createBackup = true } = options;
    ...
    try {
      ...
      if (data.customers && Array.isArray(data.customers)) {
        ...
        if (merge) {
          ...
          const existingEmails = new Set(
            this.customers.map(c => c.email.toLowerCase())
          );
          const uniqueCustomers = importedCustomers.filter((c, index) => {
            ...
            if (!isUnique) {
              console.log(
                `Filtering duplicate customer ${index + 1}:`,
                c.email,
          ...
          console.log(
            `Merging ${uniqueCustomers.length} unique customers out of ${importedCustomers.length} imported`
          );
      ...
      if (data.revenues && Array.isArray(data.revenues)) {
        ...
        let nextId = existing.length
          ? Math.max(...existing.map(r => parseInt(r.id) || 0)) + 1
          : 1;
      ...
      if (data.expenses && Array.isArray(data.expenses)) {
        ...
        let nextEId = existingE.length
          ? Math.max(...existingE.map(e => parseInt(e.id) || 0)) + 1
          : 1;
  ...
  validateImportData(data) {
    ...
    const hasValidData = Object.keys(data).some(
      key => validKeys.includes(key) && Array.isArray(data[key])
    );
    ...
    if (data.customers) {
      data.customers.forEach((customer, index) => {
        if (!customer.name || !customer.email) {
          throw new Error(
            `Customer at index ${index} missing required fields (name, email)`
          );
        }
      });
    }
    ...
    if (data.revenues) {
      data.revenues.forEach((revenue, index) => {
        if (!revenue.amount || !revenue.date) {
          throw new Error(
            `Revenue at index ${index} missing required fields (amount, date)`
          );
        }
      });
    }
    ...
    if (data.expenses) {
      data.expenses.forEach((expense, index) => {
        if (!expense.amount || !expense.date) {
          throw new Error(
            `Expense at index ${index} missing required fields (amount, date)`
          );
        }
      });
    }
